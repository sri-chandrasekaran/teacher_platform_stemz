...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // If backend signals 2FA requirement
      if (response?.twoFARequired) {
        setMfaRequired(true);
        setTempToken(response.tempToken); // backend should return a temporary token or challenge id
        setLoading(false);
        return;
      }
      ...
      // Handle HTTP 428: 2FA required
      if (error?.response?.status === 428) {
        console.log('2FA required as per server response');
        setMfaRequired(true);
