...
X_embeddings = X_embeddings / np.linalg.norm(X_embeddings, axis=1, keepdims=True) 
y = np.array(y)
...
best_rf = MultiOutputRegressor(RandomForestRegressor(**study_rf.best_params, random_state=42))
...
best_gb = MultiOutputRegressor(GradientBoostingRegressor(**study_gb.best_params, random_state=42))
